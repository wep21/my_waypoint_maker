...
void WaypointLoaderNode::initPubSub()
{
  ...
  private_nh_.param<bool>("is_pose_set", is_pose_set_, false);
  // setup publisher
  lane_pub_ = nh_.advertise<autoware_msgs::LaneArray>("/based/lane_waypoints_raw", 10, true);
  current_pose_sub_ = nh_.subscribe("/current_pose", 10, &WaypointLoaderNode::callbackFromCurrentPose, this);
}

void WaypointLoaderNode::callbackFromCurrentPose(const geometry_msgs::PoseStampedConstPtr &msg)
{
  if(!is_pose_set_){
      current_pose_ = *msg;
      ROS_INFO("current_pose is set: x=%f, y=%f, z=%f", current_pose_.pose.position.x,
               current_pose_.pose.position.y, current_pose_.pose.position.z);
      run();
      is_pose_set_ = true;
  }
}
...
void WaypointLoaderNode::createLaneWaypoint(const std::string& file_path, autoware_msgs::Lane* lane)
{
  ...
  for (auto& el: wps){
    tf::Transform tf2;
    tf::poseMsgToTF(el.pose.pose, tf2);
    tf::Transform tf_new = tf1 * tf2;
    tf::poseTFToMsg(tf_new, el.pose.pose);
  }
